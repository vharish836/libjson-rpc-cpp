 ...
 * libjson-rpc-cpp
 *************************************************************************
 * @file    httpbasicauthserver.cpp
 * @date    19.04.2018
 * @author  Harish <vharish836@gmail.com>
 * @license See attached LICENSE.txt
 ************************************************************************/

#include "httpbasicauthserver.h"
#include <cstdlib>
#include <iostream>
...
struct mhd_coninfo
{
  ...
  HttpBasicAuthServer *server;
  int code;
};

HttpBasicAuthServer::HttpBasicAuthServer(int port, const std::string &username, const std::string &password,
                                         const std::string &sslcert, const std::string &sslkey, int threads)
    : AbstractServerConnector(), port(port), threads(threads), running(false), autherr(false),
      path_sslcert(sslcert), path_sslkey(sslkey), daemon(NULL), username(username), password(password) {}

IClientConnectionHandler *HttpBasicAuthServer::GetHandler(const std::string &url)
{
...
bool HttpBasicAuthServer::StartListening()
{
  if (!this->running)
  {
...
// are deprecated.
    ...
    if (this->path_sslcert != "" && this->path_sslkey != "")
    {
      try
      {
        ...
        this->daemon = MHD_start_daemon(
            ...
            HttpBasicAuthServer::callback, this, MHD_OPTION_HTTPS_MEM_KEY,
    ...
    {
      this->daemon = MHD_start_daemon(
          mhd_flags, this->port, NULL, NULL, HttpBasicAuthServer::callback, this,
...
bool HttpBasicAuthServer::StopListening()
{
  if (this->running)
  {
...
bool HttpBasicAuthServer::SendResponse(const string &response, void *addInfo)
{
...
bool HttpBasicAuthServer::SendOptionsResponse(void *addInfo)
{
  struct mhd_coninfo *client_connection =
      static_cast<struct mhd_coninfo *>(addInfo);
...
void HttpBasicAuthServer::SetUrlHandler(const string &url,
...
HttpBasicAuthServer::FailureType HttpBasicAuthServer::ValidateCredentials(const char* user, const char* pass)
{
  if (user == NULL || pass == NULL || username.compare(user) != 0 || password.compare(pass) != 0)
  {
    if(autherr) {
      return Fatal;
    }
...
int HttpBasicAuthServer::callback(void *cls, MHD_Connection *connection, const char *url,
                                  ...
                                  void **con_cls)
{
  ...
  if (*con_cls == NULL)
  {
    ...
    client_connection->server = static_cast<HttpBasicAuthServer *>(cls);
  ...
  if (string("POST") == method)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        fail = client_connection->server->ValidateCredentials(user, pass);
        if (user != NULL)
          free(user);
        if (pass != NULL)
          free(pass);
        if (fail == Normal)
        {
          ...
          client_connection->server->SendResponse("Authentication required", client_connection);          
        }
        else if(fail == Fatal)
        {
